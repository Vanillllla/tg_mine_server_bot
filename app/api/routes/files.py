...
async def list_active_files(
    request: Request,
    path: str = "",
    current_user: AuthUser = Depends(require_permission("files.view")),
) -> dict:
    try:
...
@router.get("/text")
async def read_active_text_file(
    request: Request,
    path: str,
    current_user: AuthUser = Depends(require_permission("files.view")),
) -> dict:
...
@router.put("/text")
async def write_active_text_file(
    request: Request,
    path: str,
    payload: WriteTextFileRequest,
    current_user: AuthUser = Depends(require_permission("files.edit")),
) -> dict:
    try:
...
@router.get("/download")
async def download_active_file(
    request: Request,
    path: str,
    current_user: AuthUser = Depends(require_permission("files.download")),
) -> FileResponse:
    try:
...
@router.delete("")
async def delete_active_path(
    request: Request,
    path: str,
    current_user: AuthUser = Depends(require_permission("files.delete")),
) -> dict:
    try:
...
@router.patch("")
async def rename_active_path(
    request: Request,
    path: str,
    payload: RenamePathRequest,
    current_user: AuthUser = Depends(require_permission("files.rename")),
) -> dict:
    try:
